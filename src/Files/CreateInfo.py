...
class CreateData:
    def __init__(self, path="Saves", name=None):
        ...
        self.info = {
            "Name": {"value": str(name), "colour": Fore.RED},
            "Players": {
                "Player 1": {"value": getpass.getuser(), "colour": Fore.GREEN},
                "Player 2": {"value": "None", "colour": Fore.RED},
            },
            "Size": {
    ...
    def getGameList(self):
        return self.cln.clean(
            self.saveModule.ListFolder(self.info.get("Location").get("value"))
        )
...
"""
    ...
    def name(self, passedName: str = None) -> str:
        ...
        while gameName is None:
            ...
            if re.search(forbidden_chars_pattern, gameName):
                gameName = None
                Message.warn("Game name contains invalid characters!")
    ...
    def username(self, users: list[str] = None) -> None:
        ...
        for i, olduser in enumerate(oldusers):
            ...
            while newuser is None:
                cursor(19, 0)
                print(f"Please enter player {i + 1} name (blank to keep same)", end="")
                clearLength = SPACE * lenstr(oldusers, -1)
                print(f"{cursor(3, 0, True)}Players: [{clearLength}", end="")
...
To return to the default save location, use: ~/r!"""
            ...
            # Move curosr
            cursor(5, 0, end="")
            result, Location = self.chk.getInput(
                "Location: ", self.chk.ModeEnum.path, rCheck=True
            )
            if Location == "~/r!":
                enc = [self.saveModule.encoding.JSON, self.saveModule.encoding.BINARY]
                Location = self.saveModule.Read("Data/Settings", encoding=enc).get(
                    "path"
                )
            ...
            try:
                ...
                PrintTraceback()
                Message.warn("Error occured whilst trying to change location.", timeS=2)
                Location = None
    ...
    def MultiPlayer(self, Multiplayer: bool = None):
        if self.getInfoFieldValue("Location") == "Saves":
            Message.clear("Save location is default. Multiplayer is disabled!", timeS=2)
            return
    ...
    def SpectateChange(self, sepctate: bool = None):
        spec = sepctate
        if sepctate is None:
            spec = self.chk.getInput("Let others spectate?: ", self.chk.ModeEnum.yesno)
    ...
    def Password(self):
        ...
        while self.info.get("Password").get("true") is None:
            password = getpass.getpass("Enter a password (blank for no passwords): ")
            if password.rstrip() == "":
    ...
    def save(self):
        ...
        if password is not None:
            ...
            if check != password:
                Message.clear("Please make sure you can remember the password", timeS=2)
                return False

        # Create board
        board = Board.CreateBoard(
            self.getInfoFieldValue("Size", "X"), self.getInfoFieldValue("Size", "Y")
        )
        ...
        for user in users:
            ...
            self.saveModule.Write(
                board, f"{userLoc}/ships", encoding=self.saveModule.encoding.BINARY
            )
            self.saveModule.Write(
                placed,
                f"{userLoc}/placedData",
        ...
        data = {
            ...
            "password": self.info.get("Password").get("true"),
            "size": [
                self.getInfoFieldValue("Size", "X"),
                self.getInfoFieldValue("Size", "Y"),
            ],
            "users": users,
        }
        self.saveModule.Write(
            data, f"{gameLoc}/GameData", encoding=self.saveModule.encoding.BINARY
        )
