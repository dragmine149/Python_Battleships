...
class Place:
    ...
    def _rotationCheck(self):
        ...
        while not string:
            # Highlights important letters, shows you that you don't have to type whole word.
        ...
        while True:
            try:
                ...
                for i in range(ship.Length):
                    ...
                    # A Bunch of checks, Personally don't like these but if we don't want ships to rotate off the board, these have to be included.
                    if rotation == 0:
                        if placeId[0] - i < 0:
    ...
    def __GetLocation(self):
        # Better location check to get the location easier. And fixes an incorrect issue
        Location = [None, None]
        while Location[0] is None or Location[1] is None:
            Location = Functions.LocationConvert(input("Please enter location to place ship: "))
    ...
    def PlaceData(self, place):
        ...
        board, empty = self.AttemptPlace(backupCopy, self.ships[place], self.__GetLocation, self._rotationCheck)
        Functions.clear()
        ...
        cont = Functions.check("\nDoes this look correct? (y or n): ", returnFunc=(yes, no)).getInput('ynCheck')
