...
class save:
    ...
    data -> {
        ...
        json -> whever to encode the data in json (can't easily be edited but can be read)
    }
    Api -> Forces use of api over normal data
    """
    def __init__(self, data={name, path, Json=False}, Api=False):
    ...
    def __loadApi(self):
        ...
        # Remove api bypass?
        if (self.path.find("/") == -1 and self.path.find("\\") == -1 and self.path != "Saves") or Api:
            ...
            try:
                ...
                change = input("Please enter the new path (type 'install' to install googledrive api): ")
